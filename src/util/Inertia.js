    ...
    var Inertia = function (ele, options) {
        ...
        **/
        ...
        document.addEventListener('touchend', function () {
            ...
            // 反弹的参数  缓冲调整
            var reverseX = 0.2, reverseY = 0.2; //1
            // 速度计算法
            var step = function () {
                if (data.touching == true) {
            ...
            var edge = function () {
                ...
                // 初始值和变化量
                var initX = ele.distanceX,initY = ele.distanceY, changeX = 0,changeY = 0;
                // 判断元素现在在哪个半区
                var bound = ele.getBoundingClientRect();
                //判断是上下移动还是左右移动
                if(params.isUpDown){
                    if (bound.top + bound.height / 2 < winHeight / 2) {
                        changeY = -1 * bound.top;
                    } else {
                        changeY = winHeight - bound.bottom;
                    }
                }else{
                    if (bound.left + bound.width / 2 < winWidth / 2) {
                        changeX = -1 * bound.left;
                    } else {
                        changeX = winWidth - bound.right;
                    }
                }
                ...
                var run = function () {
                    ...
                    var x = 0,y = 0;
                    if(params.isUpDown){
                        y = easeOutBounce(start, initY, changeY, during);
                        fnTranslate(initX, y);
                    }else{
                        x = easeOutBounce(start, initX, changeX, during);
                        fnTranslate(x, initY);
                    }
                    if (start < during) {
