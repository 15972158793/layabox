...
(function rotate(){
   ...
   function createPd(){
       if(document.getElementById('preventTran') === null){
            ...
            img.src = imgData;
            pd.appendChild(img);
            img.style.margin = '40px auto -20px';//调整字和img的位置
   ...
   window.onorientationchange=function(){
        ...
        //视频模式下不执行
        var iframe = Laya.Browser.getElementById('videoframes');
        Tool.log("video iframe rotate:" + iframe);
        if(iframe != null && iframe != undefined){
            /*iframe.style.width = Laya.Browser.clientHeight + "px";
            iframe.style.height = Laya.Browser.clientWidth + "px";
            var e = Laya.Browser.getElementById('videoclose');
